...
머신러닝은 훈련 데이터에 존재하는 패턴을 암기하는 데만 사용될 수 있다는 것을 주의 <br>
...
검증 데이터에 대한 모델의 성능이 저하되기 시작하면 과대적합이 일어난 것<br>
...
+ 전체 모델이 사용자의 장치에 다운로드되므로 모델에 대해 비밀로 유지할 필요가 없도록 해야 함
...
* 입력 데이터는 최종 사용자의 컴퓨터 또는 전화기에 남아 있어야 함
* 애플리케이션에는 엄격한 지연 시간 제약 존재
* 모델이 다운로드되고 캐시된 후에도 연결 없이 계속 작동하려면 앱이 필요

<br>
<br>
**추론 모형 최적화**<br>
+ 사용 가능한 전력 및 메모리에 엄격한 제약이 있는 환경이나 대기 시간이 짧은 애플리케이션에 배포할 때 중요
+ TensorFlow.js로 가져오거나 TensorFlow Lite로 내보내기 전에 항상 모델을 최적화 필요 
